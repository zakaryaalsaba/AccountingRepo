...
async function save() {
  ...
  if (!form.value.medication_name?.trim()) fieldErrors.value.medication_name = t('clinical.validationRequired');
  ...
  const body = {
    ...
    service_fee: form.value.service_fee === '' ? undefined : Number(form.value.service_fee),
  };
  try {
    if (editingId.value) {
      await api.put(`/api/prescriptions/${editingId.value}`, body);
    } else {
...
// When selecting a visit for a new prescription, prefill doctor_id from that visit
...
<template>
  <div class="ui-page">
    <div class="ui-page-head">
      <h1 class="ui-page-title">{{ t('clinical.prescriptionsTitle') }}</h1>
      <p class="ui-page-desc">{{ t('clinical.prescriptionsSubtitle') }}</p>
    </div>
    ...
    <div class="ui-card ui-card-pad mb-8">
      <h2 class="ui-card-title mb-4">{{ editingId ? t('clinical.editPrescription') : t('clinical.addPrescription') }}</h2>
      <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <input v-model="form.medication_name" class="ui-input w-full" />
          <p v-if="fieldErrors.medication_name" class="mt-1 text-xs text-rose-600">{{ fieldErrors.medication_name }}</p>
        </div>
        <div>
          <label class="ui-label">{{ t('clinical.dosage') }}</label>
          <input v-model="form.dosage" class="ui-input w-full" />
        </div>
        <div>
          <label class="ui-label">{{ t('clinical.serviceFee') }}</label>
          <input v-model="form.service_fee" type="number" min="0" step="0.01" class="ui-input w-full" />
        </div>
        <div class="sm:col-span-2">
          <label class="ui-label">{{ t('clinical.instructions') }}</label>
          <textarea v-model="form.instructions" class="ui-input min-h-[4rem] w-full" />
        </div>
      </div>
      ...
      <table class="ui-table text-sm">
        ...
        <tbody>
          <tr v-for="row in list" :key="row.id">
            ...
            <td class="font-medium">{{ row.medication_name }}</td>
            <td>{{ row.doctor_name }}</td>
            <td>{{ row.service_fee != null ? row.service_fee : '—' }}</td>
          ...
          <tr v-else-if="!list.length">
            <td colspan="6" class="py-12 text-center text-slate-500">{{ t('clinical.noPrescriptionsYet') }}</td>
          </tr>
        </tbody>
      </table>
    </div>
